...
function ForgetPassword() {
  ...
  const {handleSubmit,  register,reset, formState: {errors}} = useForm({defaultValues:{email:""}, resolver: yupResolver(forgot_password_schema)});
  const {mutate, isPending} =useMutation({mutationKey: ["forgotPassword"], mutationFn: ForgetPasswordMutation});

  const onSubmit=(data) => {
    mutate({variables:{email: data.email}},{
              ...
              <div className="w-full input-from">
                ...
                <div className="mb-2">
                  <input
                    type="submit"
                    disabled={isPending}
                    value={isPending ? "Loading..." : "Send Password Reset Link"}
